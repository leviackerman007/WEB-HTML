...
app.get('/product/:id',(req,res)=>{
    ...
    if(id>product.length){
        ...
        res.status(200).send(product[id-1])
    }
})
...
app.get('/product',(res,req)=>{
    ...
    if(change){
        const filProduct=product.filter(product=>product.name==change)
        if(filProduct.length==0){
            res.status(404).send("No product found")
        }else{
            res.status(200).send(filProduct)
        }
    }
})

app.post("/product",isUserLogin,(req,res)=>{
    fs.writeFile("created.txt","This is created using express",(err)=>{
        if(err){
